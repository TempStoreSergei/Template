...
export type TableListItem = {
  grocery_name: string;
  category_id: number;
  grocery_life_time: number;
  grocery_in_stock: number;
  grocery_unit: number;
  grocery_img: string; // URL for the image
};
...
export const baseColumns: TableColumnItem[] = [
  ...
  },
  {
    ...
    customRender: ({ record }) => {
      return (
        <Tag>
          {`${record.grocery_life_time} ${getHourDeclension(record.grocery_life_time)}`}
        </Tag>
      );
    },
  },
  ...
  {
    title: "Есть в наличий",
    dataIndex: "grocery_in_stock",
    hideInSearch: true,
    width: 200,
    customRender: ({ record }) => {
      return (
  ...
  },
  {
    title: "Вид заготовки",
    hideInSearch: true,
    dataIndex: "grocery_img",
    width: 200,
    customRender: ({ record }) => {
      return (
        <Image
          src={getDataFromServer(record.grocery_img)}
          width={120}
          height={120}
          fallback={
