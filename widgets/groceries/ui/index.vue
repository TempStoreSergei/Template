<template>
  <a-tabs
    v-model:activeKey="activeKey"
    tab-position="right"
    @tabScroll="callback"
  >
    <template #leftExtra>
      <a-typography-text class="groceries__title" strong
        >Группы заготовок</a-typography-text
      >
      <a-divider />
    </template>
    <a-tab-pane
      v-for="(category, categoryIndex) in categories"
      :key="categoryIndex"
      :tab="category.category_name"
    >
      <a-input-search
        v-model:value="search"
        class="groceries__input"
        placeholder="Поиск по заготовкам"
        size="large"
      >
        <template #enterButton>
          <a-button>Поиск</a-button>
        </template>
      </a-input-search>
      <section class="groceries__list">
        <article
          v-for="(grocery, groceryIndex) in filteredGroceries(category)"
          :key="groceryIndex"
        >
          <a-card class="groceries__item">
            <template #cover>
              <img
                :alt="grocery.grocery_name"
                :src="getDataFromServer(grocery.grocery_img)"
                :style="{
                  objectFit: grocery.grocery_img ? 'contain' : 'cover',
                }"
                style="height: 150px"
                @click="openPrintStikerModal(grocery.grocery_name, grocery.id)"
                @error="handleImageError"
              />
            </template>
            <template #actions>
              <div style="padding: 10px">
                <a-button
                  type="primary"
                  block
                  size="large"
                  @click="
                    openPrintStikerModal(grocery.grocery_name, grocery.id)
                  "
                >
                  <template #icon> <PrinterOutlined /></template>
                  Этикетка
                </a-button>
              </div>
            </template>
            <a-card-meta
              :title="grocery.grocery_name"
              :description="
                'Срок годности: ' + grocery.grocery_life_time + ' ч'
              "
            />
          </a-card>
        </article>
      </section>
    </a-tab-pane>
  </a-tabs>
</template>

<script lang="ts" setup>
import { ref, onMounted } from "vue";
import { PrinterOutlined } from "@ant-design/icons-vue";
import { getExistedGrociries, printStikerGroceries } from "../api";
import { groceriesInfoPrint } from "../conf/formSchemas";
import { useFormModal } from "~/hooks/useModal";

const categories = ref([]);
const activeKey = ref(0);
const search = ref("");

const openPrintStikerModal = async (name: string, id: number) => {
  await showModal({
    modalProps: {
      title: `Напечатать этикетку для ${name}`,
      maskClosable: false,
      okText: "Напечатать",
      isUser: true,
      width: 700,
      onFinish: async (values) => {
        await printStikerGroceries(id, values);
      },
    },
    formProps: {
      labelWidth: 100,
      schemas: groceriesInfoPrint,
      autoSubmitOnEnter: true,
    },
  });
};

const filteredGroceries = (category) => {
  if (!search.value) {
    return category.groceries;
  }

  const searchTerm = search.value.toLowerCase();
  return categories.value
    .flatMap((cat) => cat.groceries)
    .filter((grocery) =>
      grocery.grocery_name.toLowerCase().includes(searchTerm)
    );
};

const [showModal] = useFormModal();

const fallbackImage =
  "data:image/png;base64,";

const handleImageError = (event) => {
  event.target.src = fallbackImage;
};

onMounted(async () => {
  categories.value = await getExistedGrociries();
});
</script>

<style lang="scss">
.groceries {
  &__list {
    height: calc(100% - 42px);
    display: flex;
    overflow: auto;
    flex-wrap: wrap;
    padding: 24px 0 24px 24px !important;
    align-items: flex-start;
    gap: 24px;
  }
  &__input {
    padding: 8px 24px;
  }
  &__item {
    width: 340px;
    aspect-ratio: 1.618 / 1;
  }
  &__title {
    padding: 16px 16px 0 16px;
    display: block;
  }
}
</style>
