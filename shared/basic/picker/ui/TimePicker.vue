...
const isWithinRange = (index) => {
  ...
  const distanceToScrollBeginning = Math.abs(
    index - store.scroll + sourceLength,
  );
...
const handleTouchEnd = () => {
  ...
  if (store.touchData.yArr.length > 1) {
    const [prevY, prevTime] =
      store.touchData.yArr[store.touchData.yArr.length - 2];
    const [currentY, currentTime] =
      store.touchData.yArr[store.touchData.yArr.length - 1];
...
const normalizeScroll = (scroll) => {
  ...
  return isLimit
    ? Math.min(
        Math.max(normalizedScroll, props.data.minScrollValue),
        props.data.maxScrollValue,
      )
    : normalizedScroll;
};
  ...
  return new Promise((resolve) => {
    ...
    const tick = () => {
      ...
      if (pass < time) {
        ...
        store.scroll = normalizeScroll(
          initScroll + easing[easingName](progress) * totalScrollLen,
        );
...
watch(() => props.data.value, (newValue) => {
  if (typeof newValue === 'number' && newValue >= 0 && newValue < props.data.source.length) {
    if (newValue !== store.scroll) {
      stop(); // Останавливаем анимацию
      store.scroll = newValue;
      store.selected = props.data.source[newValue];
    }
  }
});
