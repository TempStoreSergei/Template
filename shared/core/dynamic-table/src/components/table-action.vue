<template>
  ...
  <article :style="getStyle">
    <template
      v-for="(actionItem, index) in filteredActions"
      :key="generateKey(actionItem, index)"
    >
      ...
      <a-divider
        v-if="divider && index < filteredActions.length - 1"
        type="vertical"
      />
    </template>
  </article>
...
const ActionItemRender: FunctionalComponent<ActionItem> = (
  ...
  if (tooltip) {
    return h(Tooltip, getTooltipProps(tooltip), {
      default: renderWithPopConfirm,
    });
  }
