  ...
  <teleport :to="getContainer()">
    <ProConfigProvider>
      ...
      >
        ...
        >
          <template #title>
            <slot name="title">{{ $attrs.title || "Надпись" }}</slot>
          </template>
          <template #closeIcon>
            <slot name="closeIcon">
              <Space :size="32" class="ant-modal-operate" @click.stop>
                <FullscreenOutlined
                  v-if="!fullscreenModel && !isUser"
          ...
          <slot>
            ...
            ② The window can be resized from eight directions; <br />
            ③ The window can be minimized, maximized, restored, and closed;
            <br />
            ④ Window minimum width/height is restricted.
          </slot>
