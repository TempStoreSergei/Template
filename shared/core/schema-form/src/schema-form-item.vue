<template>
  <Col v-if="getShow.isIfShow" v-show="getShow.isShow" v-bind="schema.colProps">
    <Divider
      ...
      v-bind="Object.assign(getComponentProps)"
    >
    ...
    >
      ...
      <template v-if="schema.beforeSlot">
        <slot
          v-if="isString(schema.beforeSlot)"
          :name="schema.beforeSlot"
          v-bind="getValues"
        >
          <span class="before-slot-text">{{ schema.beforeSlot }}</span>
        </slot>
        <component
      ...
      <template v-if="schema.afterSlot">
        <slot
          v-if="isString(schema.afterSlot)"
          :name="schema.afterSlot"
          v-bind="getValues"
        >
          <span class="after-slot-text">{{ schema.afterSlot }}</span>
        </slot>
        <component
...
const namePath = computed<string[]>(() => {
  return isArray(schema.value.field)
    ? schema.value.field
...
const modelValueType = computed<string>(() => {
  ...
  const isCheck =
    isString(component) && ["Switch", "Checkbox"].includes(component);
...
const vnodeFactory = (
  ...
  if (isString(component)) {
    ...
    // 如果是组件对象直接return
    if (
      compKeys.some((n) => n.startsWith("_") || ["setup", "render"].includes(n))
    ) {
      return component;
    }
...
const getComponentProps = computed(() => {
  ...
  if (component !== "RangePicker" && isString(component)) {
    componentProps.placeholder ??= createPlaceholderMessage(
      component,
      getLabel.value,
    );
  }
...
const fetchRemoteData = async (
  ...
  try {
    ...
    if (
      ["Select", "RadioGroup", "CheckBoxGroup"].some((n) => n === component)
    ) {
